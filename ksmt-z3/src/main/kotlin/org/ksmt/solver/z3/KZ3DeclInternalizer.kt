...
open class KZ3DeclInternalizer(
    ...
    override fun <D : KSort, R : KSort> visit(decl: KArraySelectDecl<D, R>): FuncDecl<*> =
        z3InternCtx.internalizeDecl(decl) {
            z3Ctx.mkSelect(
                decl.arg0Sort.sample() as ArrayExpr<Sort, Sort>,
                decl.arg1Sort.sample() as Expr<Sort>
            ).funcDecl
        }
    ...
    override fun <D : KSort, R : KSort> visit(decl: KArrayStoreDecl<D, R>): FuncDecl<*> =
        z3InternCtx.internalizeDecl(decl) {
            z3Ctx.mkStore(
                decl.arg0Sort.sample() as ArrayExpr<Sort, Sort>,
                decl.arg1Sort.sample() as Expr<Sort>,
                decl.arg2Sort.sample() as Expr<Sort>
            ).funcDecl
        }
    ...
    override fun <T : KBvSort> visit(decl: KBvSignedLessOrEqualDecl<T>): FuncDecl<*> =
        z3InternCtx.internalizeDecl(decl) {
            z3Ctx.mkBVSLE(decl.arg0Sort.sample().cast(), decl.arg1Sort.sample().cast()).funcDecl
        }

    override fun <T : KBvSort> visit(decl: KBvUnsignedLessOrEqualDecl<T>): FuncDecl<*> =
        z3InternCtx.internalizeDecl(decl) {
            z3Ctx.mkBVULE(decl.arg0Sort.sample().cast(), decl.arg1Sort.sample().cast()).funcDecl
        }

    override fun <T : KBvSort> visit(decl: KBvUnsignedGreaterOrEqualDecl<T>): FuncDecl<*> =
        z3InternCtx.internalizeDecl(decl) {
            z3Ctx.mkBVUGE(decl.arg0Sort.sample().cast(), decl.arg1Sort.sample().cast()).funcDecl
        }

    override fun <T : KBvSort> visit(decl: KBvSignedGreaterOrEqualDecl<T>): FuncDecl<*> =
        z3InternCtx.internalizeDecl(decl) {
    ...
    override fun <T : KBvSort> visit(decl: KBvLogicalShiftRightDecl<T>): FuncDecl<*> =
        z3InternCtx.internalizeDecl(decl) {
    ...
    override fun <T : KBvSort> visit(decl: KBvRotateLeftIndexedDecl<T>): FuncDecl<*> =
        z3InternCtx.internalizeDecl(decl) {
    ...
    override fun <T : KBvSort> visit(decl: KBvRotateRightIndexedDecl<T>): FuncDecl<*> =
        z3InternCtx.internalizeDecl(decl) {
